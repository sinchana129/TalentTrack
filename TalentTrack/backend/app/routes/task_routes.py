...
def update_task_details(current_user, task_id):
    ...
    if assigned_to: update_data['assigned_to'] = ObjectId(assigned_to)
        
    if not update_data:
        return jsonify({'message': 'No data provided'}), 400
        
    result = db.db.tasks.update_one({'_id': ObjectId(task_id)}, {'$set': update_data})
...
def delete_task(current_user, task_id):
    result = db.db.tasks.delete_one({'_id': ObjectId(task_id)})
